  ...
  return (
    ...
    >
      <div className="container-custom flex items-center justify-between">
        ...
        >
          <img
            src="/brand/pag-logo-symbol.png"
            alt="PAG logo"
            className="w-8 h-8 object-contain transition-transform duration-300 group-hover:scale-110"
          />
          <div className="flex flex-col">
            <span className="font-sans text-xl font-semibold text-slate-900 tracking-wide">{navigationConfig.brandName}</span>
          ...
          {navLinks.map((link) => (
            ...
            >
              <button
                ...
                className="flex items-center gap-1 text-sm text-slate-700 hover:text-gold-700 transition-colors duration-300 py-2"
                role="menuitem"
              ...
              {link.dropdown && (
                <div
                  ...
                  role="menu"
                >
                  <div className="bg-white rounded-md overflow-hidden min-w-[180px] border border-slate-200 shadow-lg">
                    {link.dropdown.map((item) => (
                      <button
                        key={item.name}
                        onClick={() => scrollToSection(item.href)}
                        className="block w-full text-left px-4 py-3 text-sm text-slate-700 hover:bg-gold-100 hover:text-gold-700 transition-colors"
                        role="menuitem"
                      >
                        {item.name}
                      </button>
                ...
                {link.dropdown ? (
                  <div>
                    <button
                      ...
                      className="flex items-center justify-between w-full py-4 text-lg text-slate-900 border-b border-slate-200"
                    ...
                    >
                      {link.dropdown.map((item) => (
                        <button
                          key={item.name}
                          onClick={() => scrollToSection(item.href)}
                          className="block w-full text-left pl-12 py-3 text-slate-600 hover:text-gold-700"
                          role="menuitem"
                        >
                          {item.name}
                        </button>
                  ...
                  <button
                    ...
                    className="flex items-center gap-3 w-full py-4 text-lg text-slate-900 border-b border-slate-200 hover:text-gold-700 transition-colors"
                    role="menuitem"
                  >
                    {IconComponent && <IconComponent className="w-5 h-5 text-gold-500" />}
                    {link.name}
